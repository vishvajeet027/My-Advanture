/* ================================================================
   MODULE 7 — Dashboard
   dashboard.js — Welcome, Weather, Budget, Charts,
                  Recent Trips, Quick Actions, Expenses,
                  Favorites, Settings
   ================================================================ */
...
const TYPE_COLORS = {
  Leisure:   'linear-gradient(135deg,#0abde3,#00d2d3)',
  Adventure: 'linear-gradient(135deg,#e17055,#d63031)',
  Cultural:  'linear-gradient(135deg,#6c5ce7,#a29bfe)',
  Business:  'linear-gradient(135deg,#636e72,#2d3436)',
};
...
function renderKPIs() {
  ...
  set('kpiBudget',   '$' + totalBudget.toLocaleString());
  set('kpiFavs',     favs.length);
  set('kpiExpenses', '$' + totalExpenses.toLocaleString());
}
...
function renderBudgetChart() {
  ...
  if (trips.length === 0) { el.innerHTML = '<p style="color:#aaa;font-size:0.85rem;text-align:center;padding:20px 0;">No trips yet.</p>'; return; }
    ...
    return `
      <div class="bar-item">
        <div class="bar-fill" style="height:${pct}%;background:${GRADIENTS[i]}">
          <div class="bar-tooltip">$${Number(t.budget || 0).toLocaleString()}</div>
        </div>
...
function renderDonutChart() {
  ...
  if (expenses.length === 0) {
    svgEl.innerHTML = '<text x="100" y="108" text-anchor="middle" font-size="12" fill="#aaa">No data</text>';
    return;
  }
  ...
  svgPaths += `<text x="${cx}" y="${cy - 6}" text-anchor="middle" font-size="11" fill="#888">Total</text>
    <text x="${cx}" y="${cy + 12}" text-anchor="middle" font-size="13" font-weight="700" fill="#333">$${total.toLocaleString()}</text>`;
...
/* ================================================================
   TYPE CHART — Trip type distribution
   ================================================================ */
function renderTypeChart() {
  const el = document.getElementById('typeChart');
  if (!el) return;
  const trips = getTrips();
  if (trips.length === 0) { el.innerHTML = '<p style="color:#aaa;font-size:0.85rem;">No data yet.</p>'; return; }
  ...
  el.innerHTML = Object.entries(types).map(([type, count]) => `
    <div class="type-bar-item">
      <div class="type-bar-label">
        <span>${type}</span><span>${count} trip${count > 1 ? 's' : ''}</span>
      </div>
...
function renderRecentTrips() {
  ...
  el.innerHTML = trips.map((t, i) => `
    <div class="recent-trip-row" onclick="window.location.href='mytrips.html'">
      <div class="rtr-icon" style="background:${GRADIENTS[i % GRADIENTS.length]}">
      ...
      <div class="rtr-meta">
        <strong>${t.budget ? '$' + Number(t.budget).toLocaleString() : '—'}</strong>
    ...
    return `
      <div class="trip-card">
        ...
        <div class="trip-card-body">
          ...
          ${t.budget ? `<div class="trip-detail-row"><i class="fas fa-wallet"></i><span>Budget: <strong style="color:var(--primary)">$${Number(t.budget).toLocaleString()}</strong></span></div>` : ''}
        </div>
...
function renderExpensesSection() {
  ...
  if (badgesEl) {
    badgesEl.innerHTML = `<span class="summary-badge">Total: $${total.toLocaleString()}</span>` +
      Object.entries(cats).map(([c, v]) => `<span class="summary-badge">${c}: $${v.toLocaleString()}</span>`).join('');
  }
    ...
    <div class="dest-grid-card" onclick="window.location.href='destinations.html'">
      ...
      <div class="card-body">
        ...
        <div class="card-footer-row">
          ...
          <span class="card-price">$${Number(f.price).toLocaleString()} <span>/ person</span></span>
        </div>
      </div>
    </div>`).join('');
}
...
function renderWeather() {
  ...
  if (weather.length === 0) { el.innerHTML = '<p style="color:#aaa;">No weather data available.</p>'; return; }
